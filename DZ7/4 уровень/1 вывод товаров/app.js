...
document.querySelectorAll('button').forEach(function clickHandler(button) {
 ...
 * tablets или tv и генерирует разметку.
    ...
    switch (category) {
        case "phones":
            for (let i = 0; i < products.phones.length; i++) {
                const product = products.phones[i];
                document.querySelector('.products').insertAdjacentHTML("afterbegin", `<div class="product">
        ...
        case "tablets":
            for (let i = 0; i < products.tablets.length; i++) {
                const product = products.tablets[i];
                document.querySelector('.products').insertAdjacentHTML("afterbegin", `<div class="product">
                <div>${product.name}</div>
                <img src="${product.imageUrl}" alt="">
                <div>${product.price}</div>
